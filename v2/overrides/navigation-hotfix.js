(()=>{
  ...
  const reservedIds=new Set([
    'lceoLeftRail','lceoActionRail','lceoActionDrawer','lceoActionBackdrop','lceoNavToast'
  ]);
  ...
  function markWorkspaceRoots(){
    [...document.body.children].forEach(element=>{
      if(reservedIds.has(element.id))return;
      if(['SCRIPT','STYLE','LINK'].includes(element.tagName))return;
      if(element.id==='lceoWorkspaceFrame'){
        while(element.firstChild)document.body.insertBefore(element.firstChild,element);
        element.remove();
        return;
      }
  ...
  window.addEventListener('hashchange',()=>{
    ...
    setTimeout(markWorkspaceRoots,30);
  });

  function init(){
    closeDrawer();
    markWorkspaceRoots();
    const area=currentFromHash()||'home';
    markActive(area);

    const observer=new MutationObserver(()=>markWorkspaceRoots());
